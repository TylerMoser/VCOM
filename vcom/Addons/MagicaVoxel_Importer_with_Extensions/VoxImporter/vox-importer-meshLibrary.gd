...
func _import(source_path, destination_path, options, _platforms, _gen_files):
	var import_result = VoxImporterCommon.new().import(source_path, destination_path, options, _platforms, _gen_files);
	if import_result is int:
		return import_result
	var meshes = import_result as Array
