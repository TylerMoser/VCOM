...
func read_vox_data(source_path: String) -> Dictionary:
	fileKeyframeIds.clear()
	...
	return {
		...
		"version": version,
		"vox": vox,
		"keyframe_ids": fileKeyframeIds.duplicate(),
	}
...
func read_chunk(vox: VoxData, file: VoxFile):
	...
	match chunk_id:
		...
		'nTRN':
			...
			var node = VoxNode.new(node_id, attributes, VoxNode.NodeType.TRANSFORM);
		...
		'nGRP':
			var node_id = file.get_32();
			var attributes = file.get_vox_dict();
			var node = VoxNode.new(node_id, attributes, VoxNode.NodeType.GROUP);
			vox.nodes[node_id] = node;
		...
		'nSHP':
			var node_id = file.get_32();
			var attributes = file.get_vox_dict();
			var node = VoxNode.new(node_id, attributes, VoxNode.NodeType.SHAPE);
			vox.nodes[node_id] = node;
